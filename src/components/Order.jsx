    ...
    return(
        ...
        {data.map((order,id) => (
                <li className="hidden md:flex justify-between px-20 mt-5 font-bold text-center " key={id}>
                    ...
                    <h1 className="text-center">{order.total}</h1>
                    <h1 className={order.status
                     == 'Processing' ? 'bg-yellow-300 p-2 rounded-md'
                    :order.status == 'Completed' ? 'bg-green-300 p-2 rounded-md'
                :  'bg-purple-300 p-2 rounded-md'}>{order.status}</h1>
                    <h1 className="text-center">{order.method}</h1>
            </li>
            )
            )}
            </ul>
